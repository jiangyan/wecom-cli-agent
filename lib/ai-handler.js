...
To send a message, use bot_send_message (NOT wecom_cli msg send_message).
  - DM: bot_send_message(chatid="<userid>", content="hello", chat_type=1). The chatid is the userid (e.g. "ZhangLe"), NOT a display name.
  - Group: bot_send_message(chatid="<chatid>", content="hello", chat_type=2). The chatid is the WeCom group ID (e.g. "wr1MaBHQAA..."), NOT the group name. Get it from wecom_cli msg get_msg_chat_list.
...
const BOT_SEND_SCHEMA = {
  type: "object",
  properties: {
    chatid: { type: "string", description: "For DM: the user's userid (e.g. 'ZhangLe'). For group: the group chatid." },
    chat_type: { type: "integer", description: "1 = DM (default), 2 = group chat", enum: [1, 2] },
    content: { type: "string", description: "Message content in markdown format" },
  },
...
const anthropicTools = [
  ...
  },
  {
    ...
    description: "Send a message AS the bot to a user or group chat. Use this instead of wecom_cli msg send_message. First look up the userid via wecom_cli contact get_userlist if needed.",
    input_schema: BOT_SEND_SCHEMA,
    cache_control: { type: "ephemeral" },
  },
];
...
const openaiTools = [
  {
    ...
    name: "wecom_cli",
    description:
      "Execute a wecom-cli command. Pass category, method, and ALL command arguments as top-level properties (NOT nested in an args object). Copy the JSON fields from the skill docs examples directly. Do NOT use this for sending messages — use bot_send_message instead.",
    parameters: {
      type: "object",
      properties: {
        category: { type: "string", description: "contact, doc, meeting, msg, schedule, todo" },
  ...
  },
  {
    ...
    name: "bot_send_message",
    description:
      "Send a message AS the bot to a user or group chat. Use this instead of wecom_cli msg send_message. First look up the userid via wecom_cli contact get_userlist if needed.",
    parameters: BOT_SEND_SCHEMA,
  },
];
...
async function generateReplyOpenAI(userMessage, history = []) {
  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
    ...
    for (const fc of functionCalls) {
      ...
      console.log(`[ai-tool] ${input.category || fc.name} ${input.method || ""}`, JSON.stringify(input.args || {}));
